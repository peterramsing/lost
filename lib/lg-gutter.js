module.exports = function lgGutter(css, settings) {
  ...
  css.eachDecl( declaration => {
    if (/(\$lost-gutter)/g.test(declaration.value)) {
      var newGlobalValue = declaration.value.replace(/(\$lost-gutter)/g, gutter);
      declaration.value = newGlobalValue;
    }
    if (/(\$lost-local-gutter)/g.test(declaration.value)) {
      var newLocalValue;
      console.log(declaration.parent.nodes);

      declaration.parent.nodes.forEach( parentDeclaration => {
        if (parentDeclaration.prop === 'lost-column') {
        ...
        if (parentDeclaration.prop === 'lost-column-gutter') {
          newLocalValue = parentDeclaration.value;
        }
        declaration.value = newLocalValue;
      });

      newLocalValue = declaration.value.replace(/(\$lost-local-gutter)/g, gutter);
      declaration.value = newLocalValue;
    }
